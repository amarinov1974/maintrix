...
export function getApprovalChainLabel(amount: number): string {
  if (amount <= APPROVAL_THRESHOLDS.AM_MAX) return 'Samo AM';
  if (amount <= APPROVAL_THRESHOLDS.DIRECTOR_MAX) return 'AM → D → C2';
  return 'AM → D → C2 → BOD';
}
