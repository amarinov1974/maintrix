    ...
    <Layout screenTitle="Prijave s odobrenom procjenom">
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Tickets with Approved Cost</h1>
            <p className="text-sm text-gray-600 mt-0.5">
              Cost estimation approved by AM — create work order or archive — newest first
            </p>
          </div>
          <Link to="/amm">
        ...
        {isLoading ? (
          ...
          <ul className="space-y-2">
            {approvedCostTickets.map((ticket) => (
              <li key={ticket.id}>
                <TicketPreviewRow
                  ticket={ticket}
...
function TicketPreviewRow({ ticket, onOpen }: { ticket: Ticket; onOpen: () => void }) {
  return (
    <button
      type="button"
      className="w-full text-left p-4 rounded-lg border border-gray-200 hover:bg-green-50/50 hover:border-green-200 cursor-pointer transition focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1"
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        onOpen();
