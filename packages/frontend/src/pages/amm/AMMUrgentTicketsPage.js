...
export function AMMUrgentTicketsPage() {
    ...
    return (_jsxs(Layout, { screenTitle: "Hitne prijave", children: [_jsxs("div", { className: "max-w-4xl mx-auto space-y-4", children: [_jsxs("div", { className: "flex flex-wrap items-center justify-between gap-4", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-bold text-gray-900", children: "Hitne prijave" }), _jsx("p", { className: "text-sm text-gray-600 mt-0.5", children: "Prijave s prioritetom \u2014 sortirane od najnovijih." })] }), _jsx(Link, { to: "/amm", children: _jsx(Button, { type: "button", variant: "secondary", children: "Natrag na nadzornu plo\u010Du" }) })] }), isLoading ? (_jsx("p", { className: "text-gray-500", children: "Loading\u2026" })) : openUrgentTickets.length === 0 ? (_jsx("div", { className: "rounded-lg border border-gray-200 bg-gray-50 p-6 text-center text-gray-600", children: "Nema hitnih prijava." })) : (_jsx("ul", { className: "space-y-2", children: openUrgentTickets.map((ticket) => (_jsx("li", { children: _jsx(TicketPreviewRow, { ticket: ticket, onOpen: () => setSelectedTicketId(ticket.id) }) }, ticket.id))) }))] }), selectedTicketId != null && (_jsx(AMMTicketDetailModal, { ticketId: selectedTicketId, onClose: () => setSelectedTicketId(null) }))] }));
}
function TicketPreviewRow({ ticket, onOpen }) {
    return (_jsxs("button", { type: "button", className: "w-full text-left p-4 rounded-lg border border-gray-200 hover:bg-amber-50/50 hover:border-amber-200 cursor-pointer transition focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-1", onClick: (e) => {
            ...
            e.stopPropagation();
            onOpen();
        }, children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2 mb-2", children: [_jsxs("span", { className: "font-semibold text-gray-900", children: ["Ticket #", ticket.id] }), _jsx(Badge, { variant: "urgent", children: "URGENT" }), _jsx(Badge, { variant: getInFlightStatusBadgeVariant(ticket.currentStatus), children: formatStatus(ticket.currentStatus) }), _jsx("span", { className: "text-sm text-gray-600", children: ticket.storeName }), _jsx("span", { className: "text-sm text-gray-500", children: new Date(ticket.createdAt).toLocaleDateString() })] }), _jsx("p", { className: "text-sm text-gray-700 whitespace-pre-wrap line-clamp-2", children: descriptionPreview(ticket.originalDescription ?? ticket.description) })] }));
}
